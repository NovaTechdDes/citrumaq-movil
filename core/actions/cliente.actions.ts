...
export const getClientes = async (): Promise<Cliente[]> => {
    ...
    const filas = await conexion.getAllAsync('SELECT * FROM clientes') as Cliente[];
    return filas;
};
...
export const startAgregarCliente = async (cliente: Cliente): Promise<boolean> => {
    ...
    const res = await conexion.runAsync(`INSERT INTO clientes 
        (denominacion, domicilio, telefono, documento, observacion_cliente) VALUES ('${cliente.denominacion}', '${cliente.domicilio}', '${cliente.telefono}', '${cliente.documento}','${cliente.observacion_cliente}')`);
...
export const startDeleteCliente = async (id: number): Promise<boolean> => {
    const conexion = await db();
    const res = await conexion.runAsync('DELETE FROM clientes WHERE id = $id', { $id: id })
