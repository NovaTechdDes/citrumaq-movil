...
const FormularioMaquina = () => {
    ...
    const { closeModal, modalAbierto, maquinaSeleccionada } = useMaquinaStore();
    const { reset, control, handleSubmit } = useForm({ defaultValues: maquinaSeleccionada ?? initialState })
        ...
        <KeyboardAwareScrollView extraScrollHeight={15} enableOnAndroid={true}>
            <View className='border mx-5 border-gray-300 rounded-lg p-5 mt-5 mb-20 '>
                ...
                <View className='gap-5'>
                            ...
                            render={({ field: { onChange, value } }) => (
                                <Picker
                                    ...
                                    {clientes?.map(cliente => (
                                        <Picker.Item color={pickerStyles.item.color} value={cliente.id} label={`${cliente.denominacion} - ${cliente.telefono}`} />
                                    ))}
                                </Picker>
                            )}
                        />
                    ...
                    <View>
                        <Text className='font-semibold'>Año</Text>
                        <Controller
                            name='anio'
                            control={control}
                            render={({ field: { onChange, value } }) => (
                    ...
                    <View>
                        <Text className='font-semibold'>Industria</Text>
                        <Controller
                            name='industria'
                            control={control}
                            render={({ field: { onChange, value } }) => (
                    ...
                    <View className='flex-row justify-center gap-5 w-full'>
                        {maquinaSeleccionada ? (
                            ...
                            <Pressable onPress={handleSubmit(handleAddMaquina)} disabled={isPendingAgregar} className='border border-gray-300 px-5 bg-black py-2 rounded-lg'>
