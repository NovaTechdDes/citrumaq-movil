...
const ClienteCard = ({ cliente }: Props) => {
    ...
    const { denominacion, documento, domicilio, telefono, observacion_cliente } = cliente;
    ...
    if (!cliente.denominacion.toUpperCase().startsWith(buscador) && !cliente.telefono.toUpperCase().startsWith(buscador)) return;
    ...
    const handleDelete = () => {
        ...
        Alert.alert(
            ...
            [
                ...
                { text: 'Eliminar', style: 'destructive', onPress: () => eliminar(cliente?.id!) }
            ]
        );
    };
            ...
            <View className='gap-3 px-5 py-5'>
                ...
                <View className='flex-row justify-between'>
                    <View className='flex-row items-center gap-2'>
                        <Ionicons name='person-outline' size={20} className='rounded-full p-2  bg-gray-300' />
                        <Text className='text-black font-semibold text-xl'>{denominacion}</Text>
                    </View>
                ...
                <View className='gap-2 flex-row w-full mt-5 justify-center'>
                    ...
                    {isPending ? (
                        ...
                        <Pressable onPress={handleDelete} className='border w-[45%] justify-center border-gray-500 p-2 rounded-lg flex-row'>
